...
local function get_system_node()
  -- On Windows, trust PATH directly first (winget/official installer puts node in PATH)
  ...
  local system_paths = {
    ...
    "C:/Program Files/nodejs/node.exe", -- Official Windows installer
    "C:/Program Files (x86)/nodejs/node.exe", -- Official Windows installer (x86)
    vim.fn.expand("~/AppData/Roaming/fnm/aliases/default/node.exe"), -- fnm
    vim.fn.expand("~/AppData/Roaming/nvm/current/node.exe"), -- nvm-windows
    vim.fn.expand("~/scoop/apps/nodejs/current/node.exe"), -- Scoop
    vim.fn.expand("~/.volta/bin/node.exe"), -- Volta on Windows
    -- Linux/macOS paths
    "/opt/homebrew/bin/node", -- Homebrew on Apple Silicon
    "/usr/local/bin/node", -- Homebrew on Intel Mac or standard install
    vim.fn.expand("~/.volta/bin/node"), -- Volta's global Node (Unix)
...
local function setup_nodejs()
  ...
  if node_path ~= "" then
    local cmd = vim.fn.has("win32") == 1
      and ('"' .. node_path .. '" --version')
      or (node_path .. " --version 2>/dev/null")
  ...
  local install_hint = vim.fn.has("win32") == 1
    and "  winget install OpenJS.NodeJS"
...
local function check_node_version()
  ...
  local cmd = vim.fn.has("win32") == 1
    and ('"' .. vim.g.node_host_prog .. '" --version')
    or (vim.g.node_host_prog .. " --version 2>/dev/null")
  local version_output = vim.fn.system(cmd)
  if vim.v.shell_error ~= 0 then
    return
  end
