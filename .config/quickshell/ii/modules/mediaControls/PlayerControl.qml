...
Item { // Player instance
    ...
    property int imageVersion: 0  // Simple counter to force refresh
    ...
    onArtUrlChanged: {
        ...
        // IMMEDIATELY reset everything to prevent showing wrong image
        playerController.downloaded = false
        playerController.imageVersion++  // Force immediate refresh to clear current image
        
        if (playerController.artUrl.length == 0) {
            playerController.artFileName = "default.jpg"
            playerController.artDominantColor = Appearance.m3colors.m3secondaryContainer
            return;
        }
        
        // Calculate new filename IMMEDIATELY
        let newFileName = Qt.md5(playerController.artUrl) + ".jpg"
        playerController.artFileName = newFileName
        console.log("PlayerControl: Updated artFileName to:", playerController.artFileName)
        
        // Check if file already exists
        fileChecker.running = true
    }

    Process { // File existence checker
        id: fileChecker
        command: [ "test", "-f", artFilePath ]
        onExited: (exitCode, exitStatus) => {
            console.log("PlayerControl: File check for", artFilePath, "result:", exitCode)
            if (exitCode === 0) {
                // File exists, mark as downloaded immediately
                playerController.downloaded = true
                playerController.imageVersion++
                console.log("PlayerControl: File exists, marked as downloaded, version:", playerController.imageVersion)
            } else {
                // File doesn't exist, start download
                console.log("PlayerControl: File doesn't exist, starting download...")
    ...
    Process { // Cover art downloader
        ...
        command: [ "curl", "-sSL", targetFile, "-o", artFilePath ]
        onExited: (exitCode, exitStatus) => {
            console.log("PlayerControl: Download process exited with code", exitCode, "for file:", artFilePath)
            if (exitCode === 0) {
                // Double-check that we're still on the same URL (no race condition)  
                let expectedFileName = Qt.md5(playerController.artUrl) + ".jpg"
                if (playerController.artFileName === expectedFileName) {
                    playerController.downloaded = true
                    playerController.imageVersion++  // Increment to force refresh
                    console.log("PlayerControl: Download successful and filename matches, version:", playerController.imageVersion)
                } else {
                    console.log("PlayerControl: Download successful but filename mismatch. Expected:", expectedFileName, "Got:", playerController.artFileName)
                }
            } else {
                console.log("PlayerControl: Download failed with exit code:", exitCode)
            }
        }
    }
    ...
    ColorQuantizer {
        id: colorQuantizer
                    source: (playerController.downloaded && playerController.artFileName !== "default.jpg") ? `file://${artFilePath}` : ""
                    property int cacheVersion: playerController.imageVersion  // Force recalculation        depth: 0 // 2^0 = 1 color
    ...
    Rectangle { // Background
        ...
        Image {
            ...
            anchors.fill: parent
            source: (playerController.downloaded && playerController.artFileName !== "default.jpg") ? `file://${artFilePath}` : ""
            property int cacheVersion: playerController.imageVersion  // Force recalculation
