...
Rectangle {
    ...
    onInputTextChanged: {
        console.log("=== INPUT TEXT CHANGED SIGNAL EMITTED ===");
    }
    ...
    ColumnLayout {
        ...
        Loader {
            ...
            sourceComponent: StyledTextArea { // Input area
                ...
                background: null
                onTextChanged: {
                    console.log("=== INPUT TEXTAREA TEXT CHANGED ===");
        ...
        Loader {
            ...
            sourceComponent: StyledText { // Output area
                ...
                wrapMode: Text.Wrap
                font.pixelSize: Appearance.font.pixelSize.small
                color: root.text.length > 0 ? Appearance.colors.colOnSurfaceContainer : Appearance.colors.colSubtext
                text: root.text.length > 0 ? root.text : root.placeholderText
                
                // Debug the text binding
                onTextChanged: {
                    console.log("StyledText text changed to:", text);
                    console.log("root.text:", root.text);
                    console.log("root.text.length:", root.text.length);
                    console.log("root.placeholderText:", root.placeholderText);
                }
            }
        }
