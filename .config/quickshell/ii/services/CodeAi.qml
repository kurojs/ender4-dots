...
/**
 * Independent AI service for Code mode
 * Specialized for code examples
 */
Singleton {
    id: root
    ...
    property string systemPrompt: "You are a code example generator. When users ask about programming concepts or provide code, respond with multiple COMPLETE, WORKING code examples. Always include:\n\n1. Complete code examples (not just snippets or comments)\n2. Full function/class implementations when relevant\n3. Multiple approaches (at least 3-5 different examples)\n4. Use proper markdown code blocks with language specification\n5. Include brief inline comments\n6. Show practical, real-world usage\n7. Make examples copy-pasteable and runnable\n\nExample format:\n```java\n// Complete working example\npublic class Example {\n    public static void main(String[] args) {\n        // Your complete code here\n    }\n}\n```\n\nALWAYS provide FULL, COMPLETE, WORKING examples. No incomplete code or just comments."
    ...
    function idForMessage(message) {
        return "code_" + Date.now().toString(36) + Math.random().toString(36).substr(2, 8);
    }
    ...
    Component.onCompleted: {
        console.log("CodeAi service initialized");
    }
    ...
    function saveChat(chatName) {
        addMessage(Translation.tr("Chat saving not implemented in Code mode yet"), interfaceRole);
    }

    function loadChat(chatName) {
        addMessage(Translation.tr("Chat loading not implemented in Code mode yet"), interfaceRole);
    }
    ...
    Process {
        ...
        function parseGeminiBuffer() {
            try {
                ...
                console.log("[CodeAI] Could not parse response from stream: ", e);
        ...
        onExited: (exitCode, exitStatus) => {
            ...
            if (requester.message.content.includes("API key not valid")) {
                console.log("[CodeAI] API key not valid");
            }
        }
    }
    ...
    function sendUserMessage(message) {
        if (message.length === 0) return;
        console.log("[CodeAI] Sending message:", message);
