...
/**
 * Independent AI service for Docs mode
 * Specialized for documentation links
 */
Singleton {
    id: root
    ...
    property string systemPrompt: "You are a documentation link finder. When users ask about programming concepts, frameworks, or technologies, respond ONLY with official documentation links. You must provide AT LEAST 8 high-quality, official documentation URLs. Format your response as a numbered list with brief descriptions. Do not provide explanations, tutorials, or commentary - ONLY official documentation links from authoritative sources like official websites, MDN, GitHub repos, etc. Focus on the most up-to-date and comprehensive documentation sources. Example format:\n\n1. [Official Documentation](url) - Brief description\n2. [API Reference](url) - Brief description\n\nAlways provide 8+ links minimum."
    ...
    function idForMessage(message) {
        return "docs_" + Date.now().toString(36) + Math.random().toString(36).substr(2, 8);
    }
    ...
    Component.onCompleted: {
        console.log("DocsAi service initialized");
    }
    ...
    function saveChat(chatName) {
        addMessage(Translation.tr("Chat saving not implemented in Docs mode yet"), interfaceRole);
    }

    function loadChat(chatName) {
        addMessage(Translation.tr("Chat loading not implemented in Docs mode yet"), interfaceRole);
    }
    ...
    Process {
        ...
        function parseGeminiBuffer() {
            try {
                ...
                console.log("[DocsAI] Could not parse response from stream: ", e);
        ...
        onExited: (exitCode, exitStatus) => {
            ...
            if (requester.message.content.includes("API key not valid")) {
                console.log("[DocsAI] API key not valid");
            }
        }
    }
    ...
    function sendUserMessage(message) {
        if (message.length === 0) return;
        console.log("[DocsAI] Sending message:", message);
